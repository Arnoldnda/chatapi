...
public class ConversationUserController {
    ...
    @RequestMapping(value="/promoveToAdmin",method=RequestMethod.POST,consumes = {"application/json"},produces={"application/json"})
    public Response<ConversationUserDto> promoveToAdmin(@RequestBody Request<ConversationUserDto> request) {
        log.info("start method conversationUser/promoveToAdmin");
        Response<ConversationUserDto> response   = new Response<ConversationUserDto>();
        String        languageID = (String) requestBasic.getAttribute("CURRENT_LANGUAGE_IDENTIFIER");
        Locale locale     = new Locale(languageID, "");
        try {
            response = Validate.validateList(request, response, functionalError, locale);
            if (!response.isHasError()) {
                response = conversationUserBusiness.promoteToAdmin(request, locale);
            } else {
                log.info(String.format("Erreur| code: {} -  message: {}", response.getStatus().getCode(), response.getStatus().getMessage()));
                return response;
            }
        ...
        log.info("end method conversationUser/promoveToAdmin");
        return response;
    }

    @RequestMapping(value="/demoteFromAdmin",method=RequestMethod.POST,consumes = {"application/json"},produces={"application/json"})
    public Response<ConversationUserDto> demoteFromAdmin(@RequestBody Request<ConversationUserDto> request) {
        log.info("start method conversationUser/demoteFromAdmin");
        Response<ConversationUserDto> response   = new Response<ConversationUserDto>();
        String        languageID = (String) requestBasic.getAttribute("CURRENT_LANGUAGE_IDENTIFIER");
        Locale locale     = new Locale(languageID, "");
        try {
            response = Validate.validateList(request, response, functionalError, locale);
            if (!response.isHasError()) {
                response = conversationUserBusiness.demoteFromAdmin(request, locale);
            } else {
                log.info(String.format("Erreur| code: {} -  message: {}", response.getStatus().getCode(), response.getStatus().getMessage()));
                return response;
            }
        ...
        log.info("end method conversationUser/demoteFromAdmin");
        return response;
    }
