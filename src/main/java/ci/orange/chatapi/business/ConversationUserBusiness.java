...
public class ConversationUserBusiness implements IBasicBusiness<Request<ConversationUserDto>, Response<ConversationUserDto>> {
    ...
    /**
     * promote user to admin in conversation by using ConversationUserDto as object.
     *
     * @param request
     * @return response
     *
     */
    @Transactional(rollbackFor = {RuntimeException.class, Exception.class})
    public Response<ConversationUserDto> promoteToAdmin(Request<ConversationUserDto> request, Locale locale)  throws ParseException {
        Response<ConversationUserDto> response = new Response<ConversationUserDto>();
        try {
            log.info("----begin promoveToAdmin-----");
            ...
            for (ConversationUserDto dto : request.getDatas()) {
                ...
                if (!Utilities.areEquals(existingConversation.getTypeConversation().getCode(), "GROUP")) {
                    response.setStatus(functionalError.REQUEST_ERROR(
                            "Promotion d'admin disponible uniquement dans les groupes", locale));
                    response.setHasError(true);
                    return response;
                }
                ...
                if (!Utilities.isTrue(actorMembership.getRole())) {
                    response.setStatus(functionalError.UNAUTHORIZED(
                            "Seuls les administrateurs peuvent promouvoir des membres", locale));
                    response.setHasError(true);
                    return response;
                }
                ...
                if (userTopromote == null) {
                    response.setStatus(functionalError.DATA_NOT_EXIST("Utilisateur à promouvoir inexistant", locale));
                    response.setHasError(true);
                    return response;
                }

                // verifier si l'acteur est different de celui qu'il veux promouvoir
                if (Utilities.areEquals(actorId, userTopromote.getId())) {
                    response.setStatus(functionalError.REQUEST_ERROR(
                            "Impossible de se promouvoit soit mmême ", locale));
                    response.setHasError(true);
                    return response;
                }
                ...
                //verifier si l'utilisateur est membre actif du groupe
                ConversationUser entityToSave = conversationUserRepository.findActiveUserInConversation(
                        existingConversation.getId(), userTopromote.getId());
                if (entityToSave == null) {
                    response.setStatus(functionalError.REQUEST_ERROR("Cet utilisateur n'est pas membre actif du groupe", locale));
                    response.setHasError(true);
                    return response;
                ...
                // verifier si l'utilisateur n'est pas déja admin
                if (Utilities.isTrue(entityToSave.getRole())) {
                    response.setStatus(functionalError.REQUEST_ERROR("Cet utilisateur est déja membre administrateur.", locale));
                    response.setHasError(true);
                    return response;
                }

                entityToSave.setRole(true);
                entityToSave.setUpdatedAt(Utilities.getCurrentDate());
                entityToSave.setUpdatedBy(actorId);
                items.add(entityToSave);
            }
     ...
     */
    @Transactional(rollbackFor = {RuntimeException.class, Exception.class})
    public Response<ConversationUserDto> demoteFromAdmin(Request<ConversationUserDto> request, Locale locale)  throws ParseException {
        Response<ConversationUserDto> response = new Response<ConversationUserDto>();
        try {
            log.info("----begin demoteFromAdmin-----");
            ...
            for (ConversationUserDto dto : request.getDatas()) {
                ...
                if (!Utilities.areEquals(existingConversation.getTypeConversation().getCode(), "GROUP")) {
                    response.setStatus(functionalError.REQUEST_ERROR(
                            "Rétrogradation disponible uniquement dans les groupes", locale));
                    response.setHasError(true);
                    return response;
                }
                ...
                if (!Utilities.isTrue(actorMembership.getRole())) {
                    response.setStatus(functionalError.UNAUTHORIZED(
                            "Seuls les administrateurs peuvent rétrograder des admins", locale));
                    response.setHasError(true);
                    return response;
                }
                ...
                if (Utilities.areEquals(actorId, userTopromote.getId())) {
                    response.setStatus(functionalError.REQUEST_ERROR(
                            "Impossible de se rétrograder soi-même", locale));
                    response.setHasError(true);
                    return response;
                }
                ...
                if (!Utilities.isTrue(entityToSave.getRole())) {
                    response.setStatus(functionalError.REQUEST_ERROR("Cet utilisateur n'est pas administrateur", locale));
                    response.setHasError(true);
                    return response;
                }
